...
test.describe('UI Analysis and Improvement Workflow', () => {
  ...
  test('Capture Homepage UI State', async ({ page, browserName }) => {
    ...
    const heroAnalysis = await captureUIElement(page, 'section:first-of-type', 'hero-section');
    ...
    for (let i = 0; i < Math.min(buttonCount, 5); i++) {
      const btnAnalysis = await captureUIElement(page, `button:nth-of-type(${i + 1})`, `button-${i + 1}`);
      if (btnAnalysis) uiAnalysis.push(btnAnalysis);
    }
    
    // Capture cards/sections
    const cards = page.locator('[class*="card"], [class*="Card"], article, section > div');
  ...
  test('Analyze Mobile Responsiveness', async ({ page }) => {
    ...
    for (const viewport of viewports) {
      ...
      if (hasHorizontalScroll) {
        console.log(`⚠️ Horizontal scroll detected at ${viewport.name} viewport`);
      }
    }
  });
  ...
  test('Analyze Theme Consistency', async ({ page }) => {
    ...
    for (const theme of themes) {
      ...
      // Try to set theme
      const themeToggle = page.locator('button[aria-label*="theme" i], button:has(svg[class*="sun" i]), button:has(svg[class*="moon" i])').first();
  ...
  test('Capture User Flow Screenshots', async ({ page }) => {
    const flows = [
      { name: 'home-to-exchange', path: '/', action: 'click text=/Crypto Exchange/i', finalPath: '/cryptoexchange' },
      { name: 'home-to-donate', path: '/', action: 'click text=/Donate/i', finalPath: '/donate' }
    ];
